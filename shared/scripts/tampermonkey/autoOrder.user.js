...
(function () {
    ...
    // ============================================================================
    // DRY VERIFICATION UTILITY FUNCTIONS (STEP 2 REFACTORING)
    // ============================================================================
    
    /**
     * Captures order state with comprehensive error handling, logging, and performance tracking
     * Eliminates duplication across all execution paths
     * @param {string} symbol - Trading symbol
     * @param {string} pathName - Execution path name for logging
     * @param {string} stage - 'before' or 'after'
     * @returns {Promise<Object|null>} State object with timing metadata or null on failure
     */
    async function captureStateWithLogging(symbol, pathName, stage) {
        ...
        try {
            ...
            // Performance logging with emoji indicators
            const perfEmoji = captureTime < 100 ? '🚀' : captureTime < 500 ? '📊' : captureTime < 1000 ? '⏳' : '🐌';
            
            console.log(`📋 ${pathName}: ${stage.charAt(0).toUpperCase() + stage.slice(1)} state captured for ${symbol} ${perfEmoji} ${captureTime}ms`);
            
            // Performance alerting
            if (captureTime > 1000) {
                console.warn(`🚨 SLOW STATE CAPTURE: ${pathName} ${stage} took ${captureTime}ms (>1s threshold) for ${symbol}`);
            }
            
            // Add timing metadata to state object
            if (state) {
                state._timing = {
                    captureTime,
                    captureStage: stage,
            ...
            window.verificationMetrics = window.verificationMetrics || {
                stateCaptureTimesMs: [],
                totalCaptureAttempts: 0,
                slowCaptureCount: 0
            };
            
            window.verificationMetrics.stateCaptureTimesMs.push(captureTime);
            window.verificationMetrics.totalCaptureAttempts++;
            
            if (captureTime > 1000) {
                window.verificationMetrics.slowCaptureCount++;
            }
            
            // Keep only last 100 measurements for performance
            if (window.verificationMetrics.stateCaptureTimesMs.length > 100) {
                window.verificationMetrics.stateCaptureTimesMs = window.verificationMetrics.stateCaptureTimesMs.slice(-100);
            }
            
            return state;
        } catch (error) {
            const captureTime = Date.now() - startTime;
            console.error(`❌ ${pathName}: ${stage.charAt(0).toUpperCase() + stage.slice(1)} state capture failed after ${captureTime}ms:`, error);
            
            // Track failed capture times
            window.verificationMetrics = window.verificationMetrics || { stateCaptureTimesMs: [], totalCaptureAttempts: 0, slowCaptureCount: 0 };
            window.verificationMetrics.totalCaptureAttempts++;
    ...
    /**
     * Executes verification with standardized logging and performance tracking
     * Eliminates verification execution duplication
     * @param {Object} beforeState - Before state object
     * @param {Object} afterState - After state object  
     * @param {string} symbol - Trading symbol
     * @param {string} pathName - Execution path name for logging
     * @returns {Promise<Object>} Verification result object with timing data
     */
    async function executeVerificationWithLogging(beforeState, afterState, symbol, pathName) {
        ...
        try {
            ...
            // Calculate total verification overhead (including state capture times)
            const beforeCaptureTime = beforeState?._timing?.captureTime || 0;
            const afterCaptureTime = afterState?._timing?.captureTime || 0;
            const totalOverhead = beforeCaptureTime + afterCaptureTime + verificationTime;
            
            // Performance logging with emoji indicators
            const verifyEmoji = verificationTime < 50 ? '🚀' : verificationTime < 200 ? '📊' : verificationTime < 500 ? '⏳' : '🐌';
            const overheadEmoji = totalOverhead < 100 ? '🚀' : totalOverhead < 500 ? '📊' : totalOverhead < 1000 ? '⏳' : '🐌';
            
            console.log(`🔍 ${pathName}: Verification result for ${symbol}: ${verification.success ? 'SUCCESS' : 'FAILED'} ${verifyEmoji} ${verificationTime}ms`);
            console.log(`⚡ ${pathName}: Total verification overhead: ${overheadEmoji} ${totalOverhead}ms (capture: ${beforeCaptureTime + afterCaptureTime}ms + verify: ${verificationTime}ms)`);
            
            // Performance alerting for verification
            if (verificationTime > 500) {
                console.warn(`🚨 SLOW VERIFICATION: ${pathName} verification took ${verificationTime}ms (>500ms threshold) for ${symbol}`);
            }
            
            // Performance alerting for total overhead
            if (totalOverhead > 1000) {
                console.warn(`🚨 HIGH VERIFICATION OVERHEAD: ${pathName} total overhead ${totalOverhead}ms (>1s threshold) for ${symbol}`);
            }
            
            // Track global verification performance metrics
            window.verificationMetrics = window.verificationMetrics || {
                stateCaptureTimesMs: [],
                verificationTimesMs: [],
                totalOverheadTimesMs: [],
                totalCaptureAttempts: 0,
                totalVerificationAttempts: 0,
                slowCaptureCount: 0,
                slowVerificationCount: 0,
                highOverheadCount: 0
            };
            
            // Record verification metrics
            window.verificationMetrics.verificationTimesMs.push(verificationTime);
            window.verificationMetrics.totalOverheadTimesMs.push(totalOverhead);
            window.verificationMetrics.totalVerificationAttempts++;
            
            if (verificationTime > 500) {
                window.verificationMetrics.slowVerificationCount++;
            }
            
            if (totalOverhead > 1000) {
                window.verificationMetrics.highOverheadCount++;
            }
            
            // Keep only last 100 measurements for performance
            if (window.verificationMetrics.verificationTimesMs.length > 100) {
                window.verificationMetrics.verificationTimesMs = window.verificationMetrics.verificationTimesMs.slice(-100);
            }
            if (window.verificationMetrics.totalOverheadTimesMs.length > 100) {
                window.verificationMetrics.totalOverheadTimesMs = window.verificationMetrics.totalOverheadTimesMs.slice(-100);
            }
            
            // Add timing metadata to verification result
            verification._timing = {
                verificationTime,
                totalOverhead,
                stateCaptureTime: beforeCaptureTime + afterCaptureTime,
                pathName,
                timestamp: Date.now()
            };
            ...
            const verificationTime = Date.now() - startTime;
            console.error(`❌ ${pathName}: Verification failed after ${verificationTime}ms:`, error);
            
            // Track failed verification times
            window.verificationMetrics = window.verificationMetrics || { 
                verificationTimesMs: [], 
                totalVerificationAttempts: 0, 
                slowVerificationCount: 0 
    ...
    /**
     * Creates standardized verification failure response
     * Eliminates error response duplication
     * @param {string} error - Error message
     * @param {string} method - Method identifier (optional)
    ...
    /**
     * Generate performance metrics summary for verification overhead tracking
     * Provides insights into verification performance and identifies bottlenecks
     * @returns {Object} Comprehensive performance metrics summary
     */
    function getVerificationPerformanceMetrics() {
        const metrics = window.verificationMetrics || {
            stateCaptureTimesMs: [],
            verificationTimesMs: [],
            totalOverheadTimesMs: [],
            totalCaptureAttempts: 0,
        ...
        const captureStats = calculateStats(metrics.stateCaptureTimesMs);
        const verificationStats = calculateStats(metrics.verificationTimesMs);
        const overheadStats = calculateStats(metrics.totalOverheadTimesMs);
        
        // Calculate percentages
        const slowCaptureRate = metrics.totalCaptureAttempts > 0 
            ? Math.round((metrics.slowCaptureCount / metrics.totalCaptureAttempts) * 100) 
            : 0;
        const slowVerificationRate = metrics.totalVerificationAttempts > 0 
            ? Math.round((metrics.slowVerificationCount / metrics.totalVerificationAttempts) * 100) 
            : 0;
        const highOverheadRate = metrics.totalVerificationAttempts > 0 
            ? Math.round((metrics.highOverheadCount / metrics.totalVerificationAttempts) * 100) 
            : 0;
        
        const summary = {
            // Overall counts
            totalCaptureAttempts: metrics.totalCaptureAttempts,
            totalVerificationAttempts: metrics.totalVerificationAttempts,
            
            // State capture performance
            stateCapture: {
                ...captureStats,
                slowCount: metrics.slowCaptureCount,
                slowRate: `${slowCaptureRate}%`,
                status: slowCaptureRate > 10 ? '🔴 SLOW' : slowCaptureRate > 5 ? '🟡 WARNING' : '🟢 GOOD'
            },
            
            // Verification performance
            verification: {
                ...verificationStats,
                slowCount: metrics.slowVerificationCount,
                slowRate: `${slowVerificationRate}%`,
                status: slowVerificationRate > 10 ? '🔴 SLOW' : slowVerificationRate > 5 ? '🟡 WARNING' : '🟢 GOOD'
            },
            
            // Total overhead performance
            totalOverhead: {
                ...overheadStats,
                highCount: metrics.highOverheadCount,
                highRate: `${highOverheadRate}%`,
                status: highOverheadRate > 10 ? '🔴 HIGH' : highOverheadRate > 5 ? '🟡 WARNING' : '🟢 GOOD'
            },
            
            // Performance health summary
            healthScore: Math.max(0, 100 - (slowCaptureRate + slowVerificationRate + highOverheadRate)),
            recommendation: highOverheadRate > 10 
                ? 'Consider optimizing verification logic or increasing timeouts'
                : slowCaptureRate > 10 
                    ? 'State capture is slow, check DOM performance'
                    : slowVerificationRate > 10 
                        ? 'Verification logic may need optimization'
    ...
    /**
     * Log comprehensive performance metrics summary to console
     * Provides detailed verification performance analysis
     */
    function logVerificationPerformanceReport() {
        const metrics = getVerificationPerformanceMetrics();
        ...
        console.log(`📋 State Capture Performance: ${metrics.stateCapture.status}`);
        console.log(`   Attempts: ${metrics.totalCaptureAttempts} | Slow (>1s): ${metrics.stateCapture.slowCount} (${metrics.stateCapture.slowRate})`);
        console.log(`   Times: Min ${metrics.stateCapture.min}ms | Avg ${metrics.stateCapture.avg}ms | Max ${metrics.stateCapture.max}ms | P95 ${metrics.stateCapture.p95}ms`);
        console.log('');
        
        console.log(`🔍 Verification Performance: ${metrics.verification.status}`);
        console.log(`   Attempts: ${metrics.totalVerificationAttempts} | Slow (>500ms): ${metrics.verification.slowCount} (${metrics.verification.slowRate})`);
        console.log(`   Times: Min ${metrics.verification.min}ms | Avg ${metrics.verification.avg}ms | Max ${metrics.verification.max}ms | P95 ${metrics.verification.p95}ms`);
        console.log('');
        
        console.log(`⚡ Total Overhead Performance: ${metrics.totalOverhead.status}`);
        console.log(`   High Overhead (>1s): ${metrics.totalOverhead.highCount} (${metrics.totalOverhead.highRate})`);
        console.log(`   Times: Min ${metrics.totalOverhead.min}ms | Avg ${metrics.totalOverhead.avg}ms | Max ${metrics.totalOverhead.max}ms | P95 ${metrics.totalOverhead.p95}ms`);
        
        console.log('═══════════════════════════════════════════════════════════════');
    }

    // Export performance functions for global access
    window.getVerificationPerformanceMetrics = getVerificationPerformanceMetrics;
    window.logVerificationPerformanceReport = logVerificationPerformanceReport;

    // ============================================================================
    // VERIFICATION MONITORING AND LOGGING (STEP 5.1)
    // ============================================================================
    
    /**
     * Comprehensive verification attempt logger for monitoring and debugging
     * Provides structured logging with performance metrics and audit trail
     * @param {string} symbol - Trading symbol being verified
     * @param {Object} beforeState - State before order execution
     * @param {Object} afterState - State after order execution
     * @param {Object} verificationResult - Complete verification result object
    ...
    function logVerificationAttempt(symbol, beforeState, afterState, verificationResult, options = {}) {
        ...
        const logId = `VER_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`;
        
        // ============================================================================
        // STRUCTURED VERIFICATION LOG ENTRY
        // ============================================================================
        ...
        const verificationLog = {
            ...
            ...(executionTime !== null && { executionTimeMs: executionTime }),
            ...
            stateAnalysis: {
                ...
                beforeOrders: beforeState?.ordersCount || 0,
                afterOrders: afterState?.ordersCount || 0,
                orderChanged: (afterState?.ordersCount || 0) !== (beforeState?.ordersCount || 0)
            },
            
            // ❌ Failure Details (if applicable)
            ...(verificationResult.failureReason && {
                failureReason: verificationResult.failureReason,
                failureDescription: verificationResult.description || 'No description provided'
            })
        };
        ...
        const statusEmoji = verificationResult.success ? '✅' : '❌';
        ...
        // Main log entry
        console.log(`${statusEmoji} VERIFICATION ATTEMPT [${logId}]`);
        console.log(`🕐 ${timestamp} | 📈 ${symbol} | 🛣️ ${pathName}`);
        ...
        if (executionTime !== null) {
            const perfEmoji = executionTime < 1000 ? '🚀' : executionTime < 5000 ? '⏳' : '🐌';
            console.log(`${perfEmoji} Execution Time: ${executionTime}ms`);
        }
        
        // Verification results
        console.log(`🔍 Result: ${verificationResult.success ? 'SUCCESS' : 'FAILED'} | ${confidenceEmoji} Confidence: ${verificationResult.confidence || 'UNKNOWN'}`);
        
        // State changes summary
        const stateChanges = [];
        if (verificationLog.stateAnalysis.positionChanged) {
            stateChanges.push(`Positions: ${verificationLog.stateAnalysis.beforePositions} → ${verificationLog.stateAnalysis.afterPositions}`);
        }
        if (verificationLog.stateAnalysis.orderChanged) {
            stateChanges.push(`Orders: ${verificationLog.stateAnalysis.beforeOrders} → ${verificationLog.stateAnalysis.afterOrders}`);
        }
        ...
        if (verificationResult.requirements) {
            const reqStatus = Object.entries(verificationResult.requirements)
        ...
        // Failure details
        if (!verificationResult.success && verificationResult.failureReason) {
            console.log(`❌ Failure: ${verificationResult.failureReason} - ${verificationResult.description || 'No details'}`);
        }
        
        // Performance alerts
        if (executionTime !== null && executionTime > 5000) {
            console.warn(`🚨 PERFORMANCE ALERT: Verification took ${executionTime}ms (>5s threshold)`);
        }
        
        // ============================================================================
        // DETAILED STATE SNAPSHOTS (DEBUG MODE)
        // ============================================================================
        
        if (options.includeStateSnapshots || (!verificationResult.success && options.debugFailures !== false)) {
            console.group(`📊 State Snapshots [${logId}]`);
        ...
        // Alert on consecutive failures (if tracking is enabled)
        if (!verificationResult.success) {
            window.verificationFailureCount = (window.verificationFailureCount || 0) + 1;
            if (window.verificationFailureCount >= 3) {
                console.error(`🚨 VERIFICATION ALERT: ${window.verificationFailureCount} consecutive failures detected!`);
            }
        } else {
            window.verificationFailureCount = 0;
        }
        
        // Alert on low confidence successful verifications
        if (verificationResult.success && verificationResult.confidence === 'LOW') {
            console.warn(`⚠️ LOW CONFIDENCE SUCCESS: Verification passed but with low confidence for ${symbol}`);
        }
    ...
    window.logVerificationAttempt = logVerificationAttempt;
    ...
    async function updateSymbol(selector, value) {
            ...
            // Removed change event dispatch to prevent infinite loop - the Enter key simulation should be sufficient
    ...
    async function createBracketOrdersManual(tradeData) {
        ...
        // Enhanced DOM Order with Mandatory Verification
        async function submitOrderWithDOM(orderType, priceValue, tradeData) {
            ...
            // ====================================================================
            // STEP 2.1: MANDATORY VERIFICATION INTEGRATION - ENHANCED DOM PATH
            // ====================================================================
            
            // Capture before state using DRY utility
            const beforeState = await captureStateWithLogging(tradeData.symbol, 'Enhanced DOM', 'before');
            if (!beforeState) {
                return createVerificationFailureResponse('Before state capture failed', 'ENHANCED_DOM');
            }
            ...
            try {
                ...
                // ====================================================================
                // ORIGINAL LOGIC REPLACED WITH MANDATORY VERIFICATION
                // OLD: return { success: !ticketStillVisible, method: 'ENHANCED_DOM' };
                // NEW: Capture after state and run verification
                // ====================================================================
                
                // Capture after state and execute verification using DRY utilities
                const afterState = await captureStateWithLogging(tradeData.symbol, 'Enhanced DOM', 'after');
                if (!afterState) {
                    return createVerificationFailureResponse('After state capture failed', 'ENHANCED_DOM');
                }
                
                // Execute verification with standardized logging
                const verification = await executeVerificationWithLogging(beforeState, afterState, tradeData.symbol, 'Enhanced DOM');
                
                // Return verification result with method identifier
                return { ...verification, method: 'ENHANCED_DOM' };
                
            } catch (error) {
                console.error('Enhanced DOM submission failed:', error);
                return { success: false, error: error.message, method: 'ENHANCED_DOM' };
            }
        }
        ...
        async function submitOrder(orderType, priceValue) {
            ...
            // ====================================================================
            // STEP 2.2: MANDATORY VERIFICATION INTEGRATION - UNIFIED FRAMEWORK PATH
            // ====================================================================
            ...
            if (window.UNIFIED_TRADING_FRAMEWORK?.submitOrder) {
                ...
                // Capture before state using DRY utility
                const beforeState = await captureStateWithLogging(tradeData.symbol, 'Unified Framework', 'before');
                if (!beforeState) {
                ...
                if (beforeState) {
                    ...
                    if (result.success) {
                        ...
                        // Capture after state and execute verification using DRY utilities
                        const afterState = await captureStateWithLogging(tradeData.symbol, 'Unified Framework', 'after');
                        if (!afterState) {
                            return false; // Framework claimed success but we can't verify
                        }
                        
                        // Execute verification with standardized logging
                        const verification = await executeVerificationWithLogging(beforeState, afterState, tradeData.symbol, 'Unified Framework');
                        
                        if (!verification.success) {
                            console.error('⚠️ CRITICAL: Unified Framework claimed success but verification FAILED');
                            console.error('Framework result preserved:', result);
                        }
                        
                        // Return verification success (overrides framework success)
            ...
            // ====================================================================
            // STEP 2.3: MANDATORY VERIFICATION INTEGRATION - LEGACY PATH
            // ====================================================================
            
            // Capture before state using DRY utility
            const beforeState = await captureStateWithLogging(tradeData.symbol, 'Legacy Path', 'before');
            if (!beforeState) {
                return false; // Can't proceed without before state
            }
            ...
            // ====================================================================
            // ORIGINAL LOGIC REPLACED WITH MANDATORY VERIFICATION
            // OLD: return orderId || true;
            // NEW: Capture after state and run verification
            // ====================================================================
            
            // Capture after state and execute verification using DRY utilities
            const afterState = await captureStateWithLogging(tradeData.symbol, 'Legacy Path', 'after');
            if (!afterState) {
                return false; // Legacy claimed success but we can't verify
            }
            
            // Execute verification with standardized logging
            const verification = await executeVerificationWithLogging(beforeState, afterState, tradeData.symbol, 'Legacy Path');
            
            // Final validation - record completion based on verification result
            if (orderId && window.autoOrderValidator) {
                window.autoOrderValidator.recordOrderEvent(orderId, 'SUBMISSION_COMPLETED', {
                    orderType: orderType,
                    completionTime: Date.now(),
                    success: verification.success, // Use verification result instead of assuming success
                    verificationDetails: verification
                });
                
                console.log(`✅ Order Validation Framework: submitOrder completed for ${orderType}, Order ID: ${orderId}, Verified: ${verification.success}`);
            } else {
                console.log(`✅ DOM Intelligence: submitOrder completed for ${orderType}, Verified: ${verification.success}`);
            }
            
            // Return verification success with order ID preservation
 ...
 */
    ...
    /**
     * Internal trading execution function - DO NOT CALL DIRECTLY
     * This function executes bracket orders without verification wrapper.
     * Always use the autoOrder() wrapper which enforces mandatory verification.
     ...
     */
    async function _executeOrder(symbol, qty, action, tp, sl, tickSize) {
        console.log(`🚀 _executeOrder called: symbol=${symbol}, qty=${qty}, action=${action}, tp=${tp}, sl=${sl}, tickSize=${tickSize}`);
    ...
    // --- MAIN AUTOORDER WRAPPER WITH MANDATORY VERIFICATION ---
    /**
     * Main autoOrder function with mandatory verification wrapper
     * This is the public API that enforces order verification on ALL executions.
     * NO order can be declared successful without proper multi-source verification.
     * @param {string} symbol - Trading symbol (e.g. 'NQ', 'NQU5')
     * @param {number} quantity - Quantity to trade
     * @param {string} action - 'Buy' or 'Sell'
     * @param {number} tp - Take profit points
     * @param {number} sl - Stop loss points  
     * @param {number} price - Tick size for the instrument (legacy parameter name)
     * @returns {Promise<Object>} - Result with mandatory verification
     */
    async function autoOrder(symbol, quantity, action, tp, sl, price) {
        console.log(`🔐 autoOrder WRAPPER called: symbol=${symbol}, qty=${quantity}, action=${action}, tp=${tp}, sl=${sl}, tickSize=${price}`);
        console.log(`📋 Capturing before state for mandatory verification...`);
        
        // Step 1: Capture before state with error handling
        const beforeState = await captureStateWithLogging(symbol, 'AutoOrder Wrapper', 'before');
        if (!beforeState) {
            return createVerificationFailureResponse('Before state capture failed - cannot proceed without verification baseline', 'WRAPPER');
        }
        
        // Step 2: Call internal execution function
        let result;
        try {
            console.log(`⚙️ Calling internal _executeOrder function...`);
            result = await _executeOrder(symbol, quantity, action, tp, sl, price);
            console.log(`✅ Internal execution completed`);
        } catch (error) {
            console.error(`❌ Internal execution failed:`, error);
            return createVerificationFailureResponse(`Execution failed: ${error.message}`, 'WRAPPER');
        }
        
        // Step 3: Force verification on ALL results - even if execution claims success
        if (result.success || result === true || typeof result === 'string') {
            console.log(`🔍 Execution claims success - performing mandatory verification...`);
            
            const afterState = await captureStateWithLogging(symbol, 'AutoOrder Wrapper', 'after');
            if (!afterState) {
                console.error(`❌ CRITICAL: After state capture failed - marking order as FAILED despite execution success`);
                return createVerificationFailureResponse('After state capture failed - cannot verify order success', 'WRAPPER');
            }
            
            const verification = await executeVerificationWithLogging(beforeState, afterState, symbol, 'AutoOrder Wrapper');
            
            // Override any execution success with verification result
            return {
                ...result,
            ...
            // Execution already failed - no need to verify, but log for audit trail
            console.log(`❌ Execution already failed - no verification needed`);
            return {
                ...result,
                method: 'WRAPPER',
                enforcedVerification: false,
                verificationSkipped: 'execution_already_failed'
            };
        }
    }
    
    // Make autoOrder wrapper function globally available (public API)
    window.autoOrder = autoOrder;
    
    // Make internal execution function available for debugging (NOT for direct use)
    window._executeOrder = _executeOrder;
    
    // Make DRY utility functions globally available (for testing and internal use)
    window.captureStateWithLogging = captureStateWithLogging;
    window.executeVerificationWithLogging = executeVerificationWithLogging;
    window.createVerificationFailureResponse = createVerificationFailureResponse;
