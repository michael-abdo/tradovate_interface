...
project_root = os.path.dirname(os.path.abspath(__file__))  # Use current directory
...
def connect_to_chrome(port):
    ...
    # Reuse TradovateConnection's tab finding logic pattern with improved prioritization
    tabs = browser.list_tab()
    target_tab = None
    candidate_tabs = []
    
    print(f"DEBUG: Found {len(tabs)} total tabs, checking each one...")
    ...
    for i, tab in enumerate(tabs):
        try:
            ...
            # Check if this tab contains tradovate domain
            if "tradovate" in url:
                candidate_tabs.append((i, tab, url))
                print(f"DEBUG: Tab {i} is a Tradovate candidate: {url}")
            else:
                # Don't stop non-Tradovate tabs - they might be needed by other parts of the system
                print(f"DEBUG: Tab {i} is not a Tradovate tab, leaving it alone")
        except Exception as e:
            print(f"DEBUG: Error checking tab {i}: {e}")
            # Don't stop tabs on error - they might be recoverable or needed by other parts
            print(f"DEBUG: Leaving tab {i} alone due to check error")
    
    # Prioritize tabs: main trader.tradovate.com first, then Tab 0, then others
    if candidate_tabs:
        print(f"DEBUG: Found {len(candidate_tabs)} Tradovate candidate tabs")
        
        # First priority: Check for main trader.tradovate.com domain
        for tab_index, tab, url in candidate_tabs:
            if "trader.tradovate.com" in url and not any(frame in url for frame in ["stripe", "payment", "checkout"]):
                target_tab = tab
                print(f"DEBUG: Selected main Tradovate tab {tab_index}: {url}")
                break
        
        # Second priority: Prefer Tab 0 if it contains tradovate (usually the main tab)
        if not target_tab:
            for tab_index, tab, url in candidate_tabs:
                if tab_index == 0:
                    target_tab = tab
                    print(f"DEBUG: Selected Tab 0 as fallback: {url}")
                    break
        
        # Third priority: Any other tradovate tab
        if not target_tab and candidate_tabs:
            tab_index, target_tab, url = candidate_tabs[0]
            print(f"DEBUG: Selected first available Tradovate tab {tab_index}: {url}")
        
        # Note: Don't close unused candidate tabs as other parts of the system may reference them
        # Let Chrome manage tab lifecycle naturally to avoid "Tab has been stopped" errors
        unused_count = len(candidate_tabs) - 1 if target_tab else len(candidate_tabs)
        if unused_count > 0:
            print(f"DEBUG: Leaving {unused_count} unused Tradovate tabs open for system stability")
    else:
        print("DEBUG: No Tradovate candidate tabs found")
