...
# ============================================================================
# LOG FILTER TO SUPPRESS PYCHROME WEBSOCKET NOISE
# ============================================================================

class PyChromeSuppressor(logging.Filter):
    """Suppress harmless PyChrome WebSocket JSON decode errors that spam logs"""
    def filter(self, record):
        try:
            message = record.getMessage()
            # Suppress PyChrome tab.py JSONDecodeError exceptions in _recv_loop
            if ("pychrome" in getattr(record, 'pathname', '') and 
                "JSONDecodeError" in message and
                "_recv_loop" in message):
                return False
            # Also suppress the exception traceback itself
            if ("Exception in thread" in message and 
                "_recv_loop" in message):
                return False
            return True
        except:
...
# Apply the filter to suppress PyChrome WebSocket noise  
logging.getLogger().addFilter(PyChromeSuppressor())

# Also suppress PyChrome thread exceptions by wrapping stderr
import sys
import io

class PyChromeStderrFilter:
    """Filter stderr to suppress PyChrome JSONDecodeError exceptions"""
    ...
    def write(self, text):
        ...
        # If we have a complete traceback, check if it's a PyChrome error
        if "json.decoder.JSONDecodeError: Expecting value" in self.buffer:
            if ("pychrome/tab.py" in self.buffer and "_recv_loop" in self.buffer):
                # Suppress this entire traceback
                self.buffer = ""
                return len(text)
        
        # If buffer gets too big or we see a new exception, flush the old one
        if len(self.buffer) > 10000 or ("Exception in thread" in text and self.buffer.count("Exception in thread") > 1):
...
# Wrap stderr to filter PyChrome errors
sys.stderr = PyChromeStderrFilter(sys.stderr)
...
def get_accounts():
    print("[DEBUG] MIKE TEST - accounts endpoint modified!")
...
def test_debug():
    ...
    return jsonify({'status': 'debug', 'message': 'Debug endpoint working'})
...
def update_symbol():
    try:
        data = request.json
        print(f"[DEBUG] update_symbol endpoint called with data: {data}")
        
        # Extract parameters from request
        symbol = data.get('symbol', 'NQ')
        account_index = data.get('account', 'all')
        print(f"[DEBUG] symbol: {symbol}, account_index: {account_index}")
        
        # Check if we should execute on all accounts or just one
        if account_index == 'all':
            # For symbol updates, target the user's visible Chrome interface (port 9222)
            # instead of the background trading instances
            try:
                import pychrome
                from utils.chrome_communication import safe_evaluate, OperationType
                
                # Connect to user's main Chrome instance (port 9222)
                user_browser = pychrome.Browser(url="http://localhost:9222")
                tabs = user_browser.list_tab()
                print(f"Found {len(tabs)} tabs on user's Chrome (port 9222)")
                ...
                for i, tab in enumerate(tabs):
                    try:
                        ...
                        if "trader.tradovate.com" in url and not any(frame in url for frame in ["stripe", "payment", "checkout", "_frame"]):
                            target_tab = tab
                            print(f"Found user's visible Tradovate tab: {url}")
                            break
                        else:
                            # Don't stop non-target tabs - let Chrome manage lifecycle
                            print(f"Ignoring non-target tab: {url}")
                    except Exception as e:
                        print(f"Error checking tab {i}: {e}")
                        # Don't stop tabs on error - they might be recoverable
                        print(f"Leaving tab alone due to check error")
                
                if target_tab:
                    # First inject the updateSymbol function if it doesn't exist
                    check_function_js = "typeof updateSymbol !== 'undefined'"
                    try:
                        function_check = target_tab.Runtime.evaluate(expression=check_function_js)
                        function_exists = function_check.get("result", {}).get("value", False)
                        
                        if not function_exists:
                            print("updateSymbol function not found, injecting from autoOrder.user.js...")
                            # Read the autoOrder.user.js file to get the updateSymbol function
                            import os
                            project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                            script_path = os.path.join(project_root, 'scripts/tampermonkey/autoOrder.user.js')
                            
                            with open(script_path, 'r') as f:
                                script_content = f.read()
                            
                            # Extract just the functions we need for symbol update
                            injection_js = f"""
                            // Inject essential functions for symbol update
                            function delay(ms) {{
                            ...
                            function normalizeSymbol(symbol) {{
                                ...
                                console.log(`normalizeSymbol called with: "${{symbol}}" -> "${{normalizedSymbol}}"`);
                            ...
                            console.log('Symbol update functions injected successfully');
                            """
                            
                            # Inject the functions
                            target_tab.Runtime.evaluate(expression=injection_js)
                            print("Successfully injected updateSymbol function")
                    
                    except Exception as e:
                        print(f"Error checking/injecting updateSymbol function: {e}")
                    
                    # Execute symbol update on user's visible tab
                    js_code = f"updateSymbol('.search-box--input', normalizeSymbol('{symbol}'));"
                    
                    # Use direct execution instead of safe_evaluate to avoid health check
                    try:
                        result = target_tab.Runtime.evaluate(expression=js_code)
                        if result and result.get("result"):
                            return jsonify({
                                'status': 'success',
                                'message': f'Symbol updated to {symbol} on user interface',
                                'accounts_affected': 1,
                                'details': {'result': 'Symbol update executed', 'interface': 'user_chrome_9222'}
                            })
                        else:
                            return jsonify({
                                'status': 'error',
                                'message': f'Symbol update execution failed on user interface',
                                'details': {'error': str(result), 'interface': 'user_chrome_9222'}
                            })
                    except Exception as e:
                        return jsonify({
                            'status': 'error',
                            'message': f'Error executing symbol update: {str(e)}',
                            'details': {'error': str(e), 'interface': 'user_chrome_9222'}
                        })
                        ...
                        return jsonify({
                            'status': 'error',
                            'message': f'Failed to update symbol on user interface: {error_msg}',
                            'details': {'error': error_msg, 'interface': 'user_chrome_9222'}
                        })
                else:
                    return jsonify({
                        'status': 'error',
                        'message': 'No visible Tradovate tab found on user interface (port 9222)',
                        'details': {'error': 'no_tradovate_tab', 'interface': 'user_chrome_9222'}
                    })
                    
            except Exception as e:
                print(f"[DEBUG] Exception in user interface update: {e}")
                print(f"[DEBUG] Exception type: {type(e)}")
                import traceback
                print(f"[DEBUG] Traceback: {traceback.format_exc()}")
                # Fallback to trading instances if user interface fails
                print("Falling back to trading instances...")
                ...
                return jsonify({
                    'status': 'warning',
                    'message': f'User interface update failed, updated {accounts_affected} trading instances instead',
                    'accounts_affected': accounts_affected,
                    'details': result,
                    'fallback_used': True
                })
...
def update_trade_controls():
    print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] update_trade_controls API called")
    try:
        data = request.json
...
# ============================================================================
# VERIFICATION MONITORING API ENDPOINTS (STEP 5.3)
# ============================================================================

# Initialize verification monitor
verification_monitor = None
try:
    from utils.order_execution_monitor import VerificationMonitor
    verification_monitor = VerificationMonitor()
    print("✅ Verification monitoring initialized for dashboard")
except Exception as e:
    print(f"⚠️ Warning: Failed to initialize verification monitoring: {e}")

@app.route('/api/verification-health', methods=['GET'])
def get_verification_health():
    """Get comprehensive verification health status and metrics"""
    try:
        if not verification_monitor:
            return jsonify({
                'error': 'Verification monitoring not available',
                'timestamp': datetime.datetime.now().isoformat(),
                'health_score': 0,
                'status': 'UNAVAILABLE'
            }), 503
        ...
        health_report = verification_monitor.generate_monitoring_report()
        
        # Add dashboard-specific enhancements
        dashboard_report = health_report.copy()
        dashboard_report['dashboard_timestamp'] = datetime.datetime.now().isoformat()
        dashboard_report['monitoring_enabled'] = True
        
        # Add status classification based on health score
        health_score = health_report.get('health_score', 0)
        if health_score >= 90:
            dashboard_report['status'] = 'EXCELLENT'
            dashboard_report['status_color'] = 'green'
        elif health_score >= 75:
            dashboard_report['status'] = 'GOOD'
            dashboard_report['status_color'] = 'blue'
        elif health_score >= 50:
            dashboard_report['status'] = 'WARNING'
            dashboard_report['status_color'] = 'yellow'
        else:
            dashboard_report['status'] = 'CRITICAL'
            dashboard_report['status_color'] = 'red'
        
        return jsonify(dashboard_report)
        
    except Exception as e:
        return jsonify({
            'error': f'Failed to get verification health: {str(e)}',
            'timestamp': datetime.datetime.now().isoformat(),
            'health_score': 0,
            'status': 'ERROR'
        }), 500
...
def get_verification_metrics():
    """Get real-time verification performance metrics"""
    try:
        if not verification_monitor:
            return jsonify({
                'error': 'Verification monitoring not available',
        ...
        performance_summary = verification_monitor.get_performance_summary()
        
        # Get success/failure rates for different time windows
        metrics = {
            'timestamp': datetime.datetime.now().isoformat(),
            'performance': performance_summary,
            'success_rates': {
                'last_hour': verification_monitor.get_success_failure_rates(1),
                'last_4_hours': verification_monitor.get_success_failure_rates(4),
                'last_24_hours': verification_monitor.get_success_failure_rates(24)
            },
            'alert_status': verification_monitor._get_active_alerts(),
            'consecutive_failures': verification_monitor.consecutive_failures
        }
        ...
        return jsonify({
            'error': f'Failed to get verification metrics: {str(e)}',
            'timestamp': datetime.datetime.now().isoformat()
        }), 500

@app.route('/api/verification-failures', methods=['GET'])
def get_verification_failures():
    """Get recent verification failures with pattern analysis"""
    try:
        if not verification_monitor:
            return jsonify({
                'error': 'Verification monitoring not available',
        ...
        patterns = verification_monitor.detect_failure_patterns()
        
        # Get recent failures (last 50)
        recent_failures = []
        for attempt in list(verification_monitor.verification_attempts)[-50:]:
            if not attempt.get('is_success', True):  # Only failures
                failure_info = {
                    'timestamp': attempt['timestamp'].isoformat(),
                    'symbol': attempt.get('symbol', 'unknown'),
                    'account': attempt.get('account', 'unknown'),
                    'failure_reason': attempt.get('failureReason', 'UNKNOWN'),
                    'execution_time_ms': attempt.get('execution_time_ms', 0),
                    'confidence': attempt.get('confidence', 'NONE')
                }
                recent_failures.append(failure_info)
        
        failures_data = {
            'timestamp': datetime.datetime.now().isoformat(),
            'recent_failures': recent_failures[-20:],  # Last 20 failures for UI
            'failure_patterns': patterns,
            'total_recent_failures': len(recent_failures),
            'most_common_reasons': dict(list(verification_monitor.failure_patterns.items())[:10])
        }
        ...
        return jsonify({
            'error': f'Failed to get verification failures: {str(e)}',
            'timestamp': datetime.datetime.now().isoformat(),
            'recent_failures': [],
            'patterns': {}
        }), 500

@app.route('/api/verification-alerts', methods=['GET'])
def get_verification_alerts():
    """Get active verification alerts and alert history"""
    try:
        if not verification_monitor:
            return jsonify({
                'error': 'Verification monitoring not available',
        ...
        # Get active alerts
        active_alerts = verification_monitor._get_active_alerts()
        
        # Get alert configuration
        alert_config = {
            'thresholds': verification_monitor.alert_thresholds,
            'cooldown_minutes': verification_monitor.alert_cooldown_minutes
        }
        
        # Calculate time since last alerts
        alert_status = {}
        current_time = datetime.datetime.now()
        for alert_type, last_time in verification_monitor.last_alert_time.items():
            time_since = (current_time - last_time).total_seconds() / 60  # Minutes
            alert_status[alert_type] = {
                'last_triggered': last_time.isoformat(),
                'minutes_since': round(time_since, 1),
                'in_cooldown': time_since < verification_monitor.alert_cooldown_minutes
            }
        ...
        alerts_data = {
            ...
            'alerts_enabled': verification_monitor.alert_webhook is not None
        }
        ...
        return jsonify({
            'error': f'Failed to get verification alerts: {str(e)}',
            'timestamp': datetime.datetime.now().isoformat(),
            'active_alerts': [],
            'alert_history': []
        }), 500
...
def get_verification_history():
    """Get verification attempt history with filtering options"""
    try:
        if not verification_monitor:
            return jsonify({
                'error': 'Verification monitoring not available',
        ...
        status_filter = request.args.get('status', None)  # 'success', 'failure', or None for all
        
        # Filter verification attempts
        cutoff_time = datetime.datetime.now() - datetime.timedelta(hours=hours_back)
        filtered_attempts = []
        
        for attempt in verification_monitor.verification_attempts:
            # Time filter
            if attempt['timestamp'] < cutoff_time:
                continue
            
            # Account filter
            if account_filter and attempt.get('account', '') != account_filter:
                continue
            
            # Symbol filter
            if symbol_filter and attempt.get('symbol', '') != symbol_filter:
                continue
            
            # Status filter
            if status_filter:
                is_success = attempt.get('is_success', False)
                if status_filter == 'success' and not is_success:
                    continue
                if status_filter == 'failure' and is_success:
                    continue
            ...
            history_item = {
                ...
                'symbol': attempt.get('symbol', 'unknown'),
                'account': attempt.get('account', 'unknown'),
                'success': attempt.get('is_success', False),
                'execution_time_ms': attempt.get('execution_time_ms', 0),
                'confidence': attempt.get('confidence', 'UNKNOWN'),
                'failure_reason': attempt.get('failureReason') if not attempt.get('is_success', False) else None
            }
            filtered_attempts.append(history_item)
        ...
        # Sort by timestamp (newest first) and limit to reasonable number for UI
        filtered_attempts.sort(key=lambda x: x['timestamp'], reverse=True)
        display_attempts = filtered_attempts[:200]  # Limit to 200 for UI performance
        
        history_data = {
            'timestamp': datetime.datetime.now().isoformat(),
            'history': display_attempts,
            'summary': summary,
            'total_in_timeframe': len(filtered_attempts),
            'displayed_count': len(display_attempts)
        }
...
@app.route('/api/verification-record', methods=['POST'])
def record_verification_attempt():
    """Record a verification attempt (called by trading system)"""
    try:
        if not verification_monitor:
            return jsonify({
                'error': 'Verification monitoring not available',
        ...
        verification_monitor.record_verification_attempt(verification_data)
        ...
        return jsonify({
            'error': f'Failed to record verification attempt: {str(e)}',
            'recorded': False
        }), 500
