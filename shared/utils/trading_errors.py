...
class ErrorAggregator:
    ...
    def add_error(self, error: TradingError):
        ...
        with self._lock:
            # Handle case where error is not a TradingError (e.g., JavaScriptExecutionError)
            if not hasattr(error, 'context'):
                # Convert non-TradingError exceptions to TradingError for consistent handling
                trading_error = TradingError(
                    message=str(error),
                    severity=ErrorSeverity.ERROR,
                    category=ErrorCategory.CHROME_COMMUNICATION,
                    error_code=type(error).__name__,
                    operation="chrome_operation"
                )
                error = trading_error
